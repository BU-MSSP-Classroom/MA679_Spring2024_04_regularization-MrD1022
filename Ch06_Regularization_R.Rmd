...
points(which.min(reg.summary$cp), reg.summary$cp[a], 
       col = "red", cex = 2,pch = 20)
a<- which.min(reg.summary$bic)
...
~~~
The asterisks suggest cubic x (x3) in every model, and x, x2, x3 after cubic function, thus the best selection should contain x, x2 and x3.
R squares statistic shows nearly no increase after cubic model.
Cp, BIC suggest best model to be cubic model, while adj. R-sq pointing best fit at degree=7.
~~~
...
Your code:
```{r,echo=TRUE}
# return max 8 variables
# Forward stepwise
regfit.fwd <- regsubsets(y ~ ., data = df, nvmax = 8, method = "forward")
reg.summary <- summary(regfit.fwd)
plot(reg.summary$adjr2 ,main = "Forward", xlab = "Number of Variables",
     ylab = "Adjusted RSq", type = "l")
a <- which.max(reg.summary$adjr2)
points(which.max(reg.summary$adjr2), 
       reg.summary$adjr2[a], 
       col = "red", cex = 2, pch = 20)
plot(reg.summary$cp, xlab = "Number of Variables", main = "Forward",
     ylab = "Cp", type = "l")
a <- which.min(reg.summary$cp)
points(which.min(reg.summary$cp), reg.summary$cp[a], 
       col = "red", cex = 2,pch = 20)
a<- which.min(reg.summary$bic)
plot(reg.summary$bic , xlab = "Number of Variables", main = "Forward",
     ylab = "BIC", type = "l")
points(which.min(reg.summary$bic), 
       reg.summary$bic[a], 
       col = "red", cex = 2, pch = 20)
...
plot(reg.summary$adjr2 ,main = "Backward", xlab = "Number of Variables",
     ylab = "Adjusted RSq", type = "l")
a <- which.max(reg.summary$adjr2)
points(which.max(reg.summary$adjr2), 
       reg.summary$adjr2[a], 
       col = "red", cex = 2, pch = 20)
plot(reg.summary$cp, xlab = "Number of Variables", main = "Backward",
     ylab = "Cp", type = "l")
a <- which.min(reg.summary$cp)
points(which.min(reg.summary$cp), reg.summary$cp[a], 
       col = "red", cex = 2,pch = 20)
a<- which.min(reg.summary$bic)
plot(reg.summary$bic , xlab = "Number of Variables", main = "Backward",
     ylab = "BIC", type = "l")
points(which.min(reg.summary$bic), 
       reg.summary$bic[a], 
       col = "red", cex = 2, pch = 20)
...
~~~
We can see Adj. R-sq suggests different degree of 4 or 7 in forward/backward stepwise selections, while Cp and BIC for both methods indicating cubic is optimal.
~~~
...
Your code:
```{r,echo=TRUE}
x.lasso <- model.matrix(y ~ ., df)[, -1]
y.lasso <- df$y
grid <- 10^seq(10, -2, length = 100)
set.seed(1)
train <- sample(1:nrow(x.lasso), nrow(x.lasso) / 2)
test <- (-train)
y.test <- y.lasso[test]
lasso.mod <- glmnet(x.lasso[train , ], y.lasso[train], 
                    alpha = 1, lambda = grid) # alpha = 1: lasso; 0: ridge reg
plot(lasso.mod, main = "Lasso")

set.seed(1)
cv.out <- cv.glmnet(x.lasso[train , ], y.lasso[train], alpha = 1)
plot(cv.out)
bestlam <- cv.out$lambda.min
lasso.pred <- predict(lasso.mod , 
                      s = bestlam ,newx = x.lasso[test , ])
...
Your code:
```{r,echo=TRUE}
size<-floor(dim(College)[1]*0.7)
set.seed(123)
samp_idx<-sample(1:dim(College)[1],size,FALSE)
col_tr <- College[samp_idx,]
col_tst <- College[-samp_idx,]
```
...
Your code:
```{r,echo=TRUE}
lst.sq <- lm(Apps ~ . , col_tr)
summary(lst.sq)
pred <- predict(lst.sq, col_tst)
tst.err <- sqrt(mean((pred-col_tst$Apps)^2))
tst.err
```
...
x <- model.matrix(Apps ~ ., col_tr)[, -1]
y <- col_tr$Apps
cv <- cv.glmnet(x, y, alpha = 0)
grid <- cv$lambda.min
col.rdg <- glmnet(x, y, alpha = 0, lambda = grid)
plot(cv)
col.rdg

x_tst <- model.matrix(Apps ~ ., col_tst)[, -1]
pred <- predict(col.rdg, x_tst)
tst.err <- sqrt(mean((pred-col_tst$Apps)^2))
tst.err
```
...
grid <- cv$lambda.min
col.lasso <- glmnet(x, y, alpha = 1, lambda = grid)
plot(cv)
col.lasso

#x_tst <- model.matrix(Apps ~ ., col_tst)[, -1]
pred <- predict(col.lasso, x_tst)
tst.err <- sqrt(mean((pred-col_tst$Apps)^2))
tst.err
```
